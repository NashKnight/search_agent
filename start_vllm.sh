...
#   bash start_vllm.sh --stop          # stop background server
...
LOG_FILE="$SCRIPT_DIR/logs/start_vllm_$(date +%Y%m%d_%H%M%S).log"
...
if [[ "${1:-}" == "--logs" ]]; then
    LATEST_LOG=$(ls -t "$SCRIPT_DIR"/logs/start_vllm_*.log 2>/dev/null | head -1)
...
if $DETACH; then
    ...
    echo ""
    echo "  Stop : bash start_vllm.sh --stop"
    echo "  Logs : bash start_vllm.sh --logs"
else
